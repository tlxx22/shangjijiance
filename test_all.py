...
SITES = [
    # 新测试网站（去重后 11 个）
    {"name": "航天云商", "url": "https://bd.ispacechina.com/retrieve.do?typflag=1"},
...
def test_site(site: dict, index: int, total: int) -> dict:
    ...
    try:
            ...
            if line_str.startswith('data: '):
                try:
                    ...
                    if data.get("type") == "item":
                        ...
                        name = data.get("data", {}).get("announcementName") or data.get("data", {}).get("title", "")
                        print(f"  ✓ 获取条目: {str(name)[:40]}...")
        ...
        if final_result and final_result.get("type") == "done":
            if final_result.get("items_found", 0) > 0:
