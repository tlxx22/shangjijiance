...
configMap.put("APP_RUN_COMMAND", "sh -c 'mkdir -p /root/.cache/ms-playwright && for d in /mnt/.devops_uv_cache/venv/browsers/*/; do [ -d \"\$d\" ] && ln -sf \"\$d\" /root/.cache/ms-playwright/; done && for d in /root/.cache/ms-playwright/chromium-*/; do [ -d \"\${d}chrome-linux64\" ] && ln -sf \"\${d}chrome-linux64\" \"\${d}chrome-linux\"; done; exec /mnt/.devops_uv_cache/venv/bin/gunicorn -c gunicorn.conf.py app:app'")  // 程序启动命令
