...
configMap.put("BUILD_COMMAND", "uv pip install -r requirements.txt && uvx browser-use install")  // 构建命令：和原 Dockerfile 一致
configMap.put("APP_RUN_COMMAND", "gunicorn -c gunicorn.conf.py app:app")  // 程序启动命令
configMap.put("DOCKERFILE_BASE_IMAGE", "yaowu-registry.cn-shanghai.cr.aliyuncs.com/pulic/crawler-agent-base:stable")  // 爬虫基础镜像（含 Chrome + 中文字体）
