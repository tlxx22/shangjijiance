...
configMap.put("BUILD_COMMAND", "uv sync && PLAYWRIGHT_DOWNLOAD_HOST=https://npmmirror.com/mirrors/playwright PLAYWRIGHT_BROWSERS_PATH=\$UV_PROJECT_ENVIRONMENT/browsers uv run browser-use install")  // 浏览器安装到 venv 目录
configMap.put("APP_RUN_COMMAND", "sh -c 'PLAYWRIGHT_BROWSERS_PATH=\$UV_PROJECT_ENVIRONMENT/browsers exec \$UV_PROJECT_ENVIRONMENT/bin/gunicorn -c gunicorn.conf.py app:app'")  // 程序启动命令
