...
configMap.put("BUILD_COMMAND", "uv sync && PLAYWRIGHT_DOWNLOAD_HOST=https://npmmirror.com/mirrors/playwright uv run browser-use install")  // 构建命令（使用淘宝镜像下载 Chrome）
configMap.put("APP_RUN_COMMAND", "\$UV_PROJECT_ENVIRONMENT/bin/gunicorn -c gunicorn.conf.py app:app")  // 程序启动命令
