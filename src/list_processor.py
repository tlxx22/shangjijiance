...
2. **信息类型**：点击"不限"（获取所有类型的公告）
    ...
    - 禁止点击左侧栏目导航、栏目树、公告类型树或目录树；不要为了抓取而主动切换到别的栏目上下文。
3. **日期筛选**（需判断筛选字段类型）：
   - **首先观察**筛选控件的标签：是"发布日期"还是"截止日期/开标时间"？
   - **如果是"发布日期"类筛选**：
     - 优先点击快捷按钮（如"近一天"、"近三天"）
     - 如果没有快捷按钮，先点击日期输入框，观察是否会弹出日期控件
     - 如果点击输入框后弹出日期控件，再使用日历选择 {start_date} 到 {end_date}；对于日期范围控件，第一下点击开始日期，第二下点击结束日期
     - 如果点击输入框没有弹出日期控件，再尝试点击输入框旁边的日历图标；只有输入框和日历图标都无法唤起日期控件时，才允许直接输入日期
...
- 除“全部/不限”、日期筛选、搜索/查询、翻页、打开详情页外，禁止点击左侧栏目导航、栏目树、公告类型树、目录树等会改变当前列表上下文的控件
- 如果完成筛选/查询后当前列表直接显示“暂无数据”“无数据”“没有相关记录”等空结果提示，视为当前列表上下文无匹配数据；不要切换左侧栏目或其它类别，直接结束当前上下文处理
...
如果页面上是“公告类型：”这种横向 Tab（例如：全部/招标预告/招标公告/…），请点击“全部”（等价于“不限”），确保不遗漏任何类型。
禁止点击左侧栏目导航、栏目树、公告类型树或目录树，不要主动切换到别的栏目上下文。

**3. 日期筛选（如有）：**
- **优先点击快捷按钮**：如"近一天"、"近三天"、"今天"等按钮（推荐）
- 如果没有快捷按钮，先点击日期输入框，观察是否会弹出日期控件
- 如果点击输入框后弹出日期控件，优先通过日历完成选择；如果是范围控件，第一下点开始日期，第二下点结束日期
- 如果点击输入框没有弹出日期控件，再尝试点击日期输入框旁边的日历图标
- 只有输入框和日历图标都无法唤起日期控件、且确认输入框支持纯文本编辑时，最后才尝试直接输入日期 {start_date} 到 {end_date}
- ?? 选完后检查年份是否正确（不要选成上一年）
...
- 只允许在当前已打开的列表上下文里操作：点击“全部/不限”、设置日期、点击“搜索/查询”、打开详情；禁止点击左侧栏目导航、栏目树、公告类型树、目录树
- 如果筛选/查询后页面显示“暂无数据”“无数据”“没有相关记录”，这就是当前列表上下文的最终结果；不要点击左侧栏目导航或切换类别重试
...
优先级顺序：先点快捷按钮；没有快捷按钮时，先点日期输入框看是否弹出日期控件；若弹出范围日历，第一下点开始日期、第二下点结束日期；若未弹出，再点日历图标；只有输入框和日历图标都无法唤起日期控件时，最后才直接输入日期。
完成日期筛选后再分析筛选结果。
...
- 禁止点击"下一页"、"下页"、">"、页码数字
- 禁止点击任何分页按钮或链接
- 如果当前页面没有符合条件的条目，或筛选/查询后页面显示“暂无数据”“无数据”“没有相关记录”，直接返回 {{"found": false}}
- 翻页由外部程序控制，你只负责当前页面！
- 禁止点击左侧栏目导航、栏目树、公告类型树、目录树，不要主动切换到别的栏目上下文
...
4. **禁止点击左侧栏目导航、栏目树、公告类型树、目录树** - 不要主动切换到别的栏目上下文！
