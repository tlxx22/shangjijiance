...
""".strip()
	...
	if include_is_equipment:
			system_prompt += (
				...
				"- Highest-priority exclusions: if the project is clearly about maintenance / overhaul / repair / servicing for existing equipment, output false.\n"
				"  Examples include: 维修、检修、修理、大修、保养服务, and '中煤大同能源公司塔山煤矿交流变频电牵引采煤机维修结果公告'.\n"
				"  Even if equipment names appear, do NOT output true. If the notice is about buying the equipment itself and maintenance is only ancillary to that purchase, this exclusion does not apply.\n"
				"- Highest-priority exclusions: if the purchased target / subject is a non-whole-machine item, part, accessory, spare part, component, consumable, wearing part, or auxiliary material of equipment, output false.\n"
				"  Examples include: 配件、附件、备件、零件、零部件、部件、组件、辅件、耗材、易损件、套、罩、护套、防护套、保温套、保护套、隔热套、筛网、滤芯、阀芯、密封件、管件、接头、钢丝绳.\n"
				"  Even if a whole-machine name appears before the item name (for example 混凝土罐车保温防护套, HZS120型砼拌站筛网), do NOT treat it as whole-equipment procurement; output false.\n"
	...
	if include_estimated_amount:
			...
			"  2) Otherwise, if the notice clearly gives a project-level total budget / procurement budget / overall budget, use that total amount for the WHOLE notice.\n"
			"     If only one exact total budget is known, output \"x~x\".\n"
			"     If both a total project budget and several package / lot budgets appear together, the total project budget has higher priority.\n"
			"     In that situation, NEVER use sibling package budgets as the lower/upper ends of one range.\n"
				"  3) Otherwise, if procurement items / BOQ / service scope exist (标的物/采购清单/服务范围),\n"
				"     you MUST output a NON-empty reasonable TOTAL amount in yuan as a range \"lo~hi\".\n"
				"     If there are several package / lot budgets but no explicit whole-project total budget, reason about the WHOLE notice amount from the combined package scope.\n"
				"     Do NOT turn one package budget into the lower bound and another package budget into the upper bound of the same range.\n"
				...
				"  4) Otherwise output empty string (do not guess).\n"
