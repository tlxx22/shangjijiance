...
def generate_extract_prompt(
	...
	*,
	...
	if stage == "meta":
		...
		lines.append("- subjects 必须尽量只保留设备/物资主名称；如果原文把吨级、型、尺寸、平台高度、额定载重量、功率、斗容等规格直接写在设备名前面或中间，必须把这些规格从 subjects 中拆出来放到 models")
		lines.append("- models 不仅可以填写厂家正式型号，也可以填写该单条标的物的吨级/型/规格/描述；不要因为它不是标准型号编码就留空")
		lines.append("- 示例：原文“1台50吨级挖掘机” => subjects=\"挖掘机\"，models=\"50吨级\"；不要写成 subjects=\"50吨级挖掘机\" 且 models=\"\"")
		lines.append("- 示例：原文“50型轮式装载机” => subjects=\"轮式装载机\"，models=\"50型\"；原文“26m平台高度高空作业平台” => subjects=\"高空作业平台\"，models=\"26m平台高度\"")
		...
		lines.append("- 若 subjects 先明确点名某个整机/完整设备，再跟“含…/带…/配…/及附属设备/及配套设备/及附属设施/含附件/含备品备件”等打包范围描述，应仍将前面的整机名称视为主标的，不要仅因这些尾部描述就返回空")
		lines.append("- 例如 subjects=风力发电机组（含塔筒及法兰）及附属设备，这里的主标的是“风力发电机组”这类整机，不能因为出现“含塔筒及法兰/及附属设备”就把它误判成纯附件或非整机")
		lines.append("- 强化示例：若 subjects=风力发电机组（含塔筒及法兰）及附属设备，且表中存在“风力发电机组”，则应返回“风力发电机组”；若表中没有“风力发电机组”但有最接近的整机候选（如“风力发电机”），也应返回该最接近整机候选，而不是返回空")
