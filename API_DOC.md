...
- `parentOrgName` ????????????? trim???????????????
- `confidence` ? `0~1` ???????
- `sources` ??????????? Web Search API ??????? `sourceUrls` ?????????????????????
- ???????? `responses.create` ???????????? `chat/completions` + ?? function tool?
- ???
  - `ROUTE="openai"`??? `OPENAI_BASE_URL` + `OPENAI_API_KEY` + `OPENAI_MODEL`
  - `ROUTE="sany"`??? `SANY_AI_GATEWAY_BASE_URL` + `SANY_AI_GATEWAY_KEY` + `SANY_EXTRACT_MODEL`??? `SANY_PARENT_ORG_MODEL` ??????? `X-ai-server`
  - `ROUTE="official"`??????????????? 500
- ???????????? `POST https://api.bochaai.com/v1/web-search`????? `BOCHA_API_KEY`
